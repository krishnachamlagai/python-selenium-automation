...
def click_on_target_terms_and_conditions_link(context):
    context.login_page.click_on_target_tac_link()


@when('Switch to the newly opened window')
def switch_to_newly_opened_window(context):
    context.target_app.switch_to.window(context.original_window)


@then('Verify Terms and Conditions page is opened')
def verify_terms_and_conditions_page(context):
    context.target_app_page.verify_tac_page_opened()


@then('User can close new window and switch back to original')
def switch_to_original(context):
    context.target_app_page.switch_window_by_id(context.original_window)
