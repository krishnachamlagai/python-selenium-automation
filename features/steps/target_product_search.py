...
def input_keyword_into_target_search_field(context, search_query):
    ...
    context.driver.find_element(*SEARCH_ELEMENT).send_keys(search_query)
...
def product_results_for_keyword(context, keyword):
    context.wait.until(ec.presence_of_element_located(SEARCH_RESULT_HEADING_TEXT))
    actual_text = context.driver.find_element(*SEARCH_RESULT_HEADING_TEXT).text
